#define _CRT_SECURE_NO_WARNINGS // For localtime on MSVC
#define NOMINMAX
...
bool ScheduledScanner::Initialize() {
    ...
    if (m_logger) {
            m_logger->LogFormat(LogLevel::Info, L"Initializing Scheduled Scanner...");
    }
    ...
    try {
        ...
        if (m_logger) {
           size_t count = 0;   
            m_logger->LogFormat(LogLevel::Info,  L"Scheduled Scanner initialized successfully (%zu)", count);
        }
...
void ScheduledScanner::Shutdown() {
    ...
    if (m_logger) {
        ...
        m_logger->LogFormat(LogLevel::Info, L"Shutting down Scheduled Scanner...");
    }
    ...
    if (m_logger) {
        ...
        m_logger->LogFormat(LogLevel::Info, L"Scheduled Scanner shutdown complete");
    }
}
...
bool ScheduledScanner::TriggerScanNow(const std::wstring& scanType) {
    if (m_scanInProgress.load()) {
        if (m_logger) {
            ...
            m_logger->LogFormat(LogLevel::Warning, L"Cannot trigger scan - scan already in progress");
        }
        return false;
    }
...
void ScheduledScanner::SchedulerThread() {
    if (m_logger) {
        ...
        m_logger->LogFormat(LogLevel::Info, L"Scheduled Scanner thread started");
    }
    ...
    if (m_logger) {
        ...
        m_logger->LogFormat(LogLevel::Info, L"Scheduled Scanner thread stopped");
    }
}
...
void ScheduledScanner::ExecuteScan(const std::wstring& scanType) {
    if (m_scanInProgress.exchange(true)) {
        if (m_logger) {
            ...
            m_logger->LogFormat(LogLevel::Warning, L"Scan already in progress, skipping");
        }
        return;
    }
    ...
    try {
        if (!m_scanner) {
            if (m_logger) {
                ...
                m_logger->LogFormat(LogLevel::Error, L"Scanner not available");
            }
